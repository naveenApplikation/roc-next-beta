...
const RocPlacesModal: React.FC<ModalProps> = ({
  dataImage,
  reservationModal,
  data,
}) => {
  ...
  const formattedValues = () => {
    if (Array.isArray(data.acf?.types)) {
      return data.acf?.types.map((item: any) => item.label).join(" | ");
    } else {
      return data?.acf?.types.label;
    }
  };
      ...
      <ResturatContainer>
        <ResturatWrapper>
          <p style={{  fontSize: "16px",
              ...
              fontWeight: "700",}}>{formattedValues()}</p>
        </ResturatWrapper>
      </ResturatContainer>
      <ItemImageContainer>
