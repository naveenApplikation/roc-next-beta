...
export default function ImageLayout({
  ...
  return (
    <>
      <div
        onClick={() => {
          navigate(data.link_type, data);
        }}
        className="relative cursor-pointer h-[200px] w-full rounded-[16px]"
      >
        <Image
          src={data.img_url}
          alt=""
        ...
        <div className="flex flex-row gap-[4px] box-border absolute bottom-[0px] w-full h-max  p-[12px] rounded-b-[16px]">
          <p className="text-white grow font-[600] text-[14px] overflow-hidden  text-ellipsis line-clamp-2">
            {children}
          </p>
          <Image height={10} width={8} src={FrontArrow} alt=""></Image>
        </div>
      </div>
    </>
  );
