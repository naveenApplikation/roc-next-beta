...
const ListingItems = ({ data, title }: { data: any[]; title: string }) => {
  ...
  const navigate = (item: any) => {
    if (title == "food") {
      router.push(
        `/screens/${item.category.listName}?categoryID=${item.category._id}`
      );
    } else if (title == "shopping") {
      router.push(`/screens/${item.title}?categoryID=${item.categoryId}`);
    } else {
      router.push(
        "/eventCategory/" + handleEventEncoding("encode", item?.listName)
      );
    }
  };
  return (
            ...
            >
              <div className="relative border-none flex items-center grow gap-[16px] py-[12px]">
                <div className="h-[30] w-[30] flex justify-center items-center">
                  <DynamicIcon
                    iconName={item?.xMasIcon ? item.xMasIcon : item.icon}
                    library={item?.iconPackage}
                  ></DynamicIcon>
                </div>

                <p className="absolute text-[18px] left-[50px] font-[400]">
                  {item?.listName ? item.listName : item.title}
                </p>
              </div>
              <Image
                height={10}
