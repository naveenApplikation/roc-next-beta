    ...
    const router=useRouter()
      ...
      return data.map((item:any)=>{
             return <>
                          ...
                          <div key={item} onClick={()=>{navigate(item.link_type,item)}} className="relative h-[150px] cursor-pointer w-full rounded-[16px]">
                          <Image 
                                src={item.img_url}
                                alt=""
                                height={500}
                         ...
                         {item.date &&     <div className="w-max absolute top-2 rounded-[16px] py-[5px] px-[12px] left-2 bg-[#F40035]">
                          ...
                          {/* <div className="w-full absolute bottom-[42px] h-[25px] w-full backdrop-blur-[30px] backdrop-brightness-[0.5] border-none opacity-[0.3]  "></div> */}
                            ...
                            <p className="text-white text-[12px] absolute bottom-[5px] left-[12px] font-[600] text-wrap overflow-hidden text-ellipsis line-clamp-2">{item.title}</p>
